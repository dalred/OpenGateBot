    ...
    if action == "approve":
        if set_user_approval_status(user_id, "yes"):
            ...
            await context.bot.send_message(
                chat_id=int(user_id),
                text=(
                    "ℹ️ Как работает калитка:\n\n"
                    "– Нажмите «Открыть», чтобы начать движение.\n"
                    "– Калитка автоматически дойдёт до конца, затем закроется.\n"
                    ...
                    "После завершения движения бот сам обновит состояние и покажет «замочек», когда калитка вернётся в режим ожидания.\n\n"
